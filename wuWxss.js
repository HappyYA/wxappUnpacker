  ...
  function cssRebuild(data) {
    ...
    function makeup(data) {
      ...
      if (onlyTest) {
        ...
        if (
          !actualPure[data] &&
          !blockCss.includes(wu.changeExt(wu.toDir(cssFile, frameName), ''))
        ) {
      ...
      if (isPure && actualPure[data] != cssFile) {
        if (actualPure[data])
          return (
            '@import "' +
            wu.changeExt(wu.toDir(actualPure[data], cssFile), '.wxss') +
            '";\n'
          );
        else {
          res.push(
  ...
  function preRun(dir, frameFile, mainCode, files, cb) {
    ...
    for (let name of files) {
      if (name != frameFile) {
        wu.get(name, code => {
          code = code.replace(
            /display:-webkit-box;display:-webkit-flex;/gm,
            ''
          );
